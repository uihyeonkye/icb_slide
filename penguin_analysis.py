...
plt.savefig('images/hist_bill_length.png')
plt.close()
...
plt.savefig('images/hist_bill_depth.png')
plt.close()
...
plt.savefig('images/hist_flipper_length.png')
plt.close()
...
plt.savefig('images/hist_body_mass.png')
plt.close()
...
plt.savefig('images/box_bill_length_species.png')
plt.close()

# Visualization 6: Boxplot of bill_depth_mm by species
plt.figure(figsize=(8, 6))
sns.boxplot(x='species', y='bill_depth_mm', data=penguins)
plt.title('종별 부리 깊이 박스플롯')
plt.savefig('images/box_bill_depth_species.png')
plt.close()
...
plt.savefig('images/scatter_bill_length_depth.png')
plt.close()
...
plt.savefig('images/scatter_flipper_mass.png')
plt.close()

# Visualization 9: Bar chart of species count (with cross-tab and pivot)
plt.figure(figsize=(8, 6))
species_count = penguins['species'].value_counts()
sns.barplot(x=species_count.index, y=species_count.values)
plt.title('종별 개수 막대그래프')
plt.savefig('images/bar_species_count.png')
plt.close()

# Cross-tabulation for species and island
cross_tab = pd.crosstab(penguins['species'], penguins['island'])
print("\nCross-tabulation (Species vs Island):")
print(cross_tab)

# Pivot table for mean body_mass_g by species and sex
...
# Visualization 10: Violin plot of body_mass_g by species
plt.figure(figsize=(8, 6))
sns.violinplot(x='species', y='body_mass_g', data=penguins)
plt.title('종별 체중 바이올린 플롯')
plt.savefig('images/violin_body_mass_species.png')
plt.close()

# Visualization 11: Pair plot
plt.figure(figsize=(10, 10))
sns.pairplot(penguins, hue='species', diag_kind='kde')
plt.suptitle('페어 플롯', y=1.02)
plt.savefig('images/pairplot.png')
plt.close()

# Visualization 12: Heatmap of correlations
plt.figure(figsize=(8, 6))
...
## Analysis Summary
- The dataset has {len(penguins)} penguins after dropping missing values.
- Species distribution: {species_count.to_dict()}
- Correlations show relationships between physical measurements.
"""

# Save to Markdown file
with open('penguin_analysis_report.md', 'w') as f:
    f.write(markdown_content)

print("Analysis complete. Report saved to penguin_analysis_report.md")